...
export default function Home() {
  ...
  return (
    <div className="flex flex-col min-h-screen">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-24">
            ...
            <div className="hidden md:flex items-center space-x-8">
              <button onClick={() => scrollToSection('home')} className="text-brand-charcoal/70 hover:text-[var(--color-brand-sage)] font-medium transition-colors">Home</button>
              <button onClick={() => scrollToSection('model')} className="text-brand-charcoal/70 hover:text-[var(--color-brand-sage)] font-medium transition-colors">Our Model</button>
              <button onClick={() => scrollToSection('about')} className="text-brand-charcoal/70 hover:text-[var(--color-brand-sage)] font-medium transition-colors">About Us</button>
              <button onClick={() => scrollToSection('contact')} className="text-brand-charcoal/70 hover:text-[var(--color-brand-sage)] font-medium transition-colors">Contact</button>
              <button
                onClick={() => scrollToSection('apply')}
                className="bg-[var(--color-brand-sage)] hover:bg-brand-sage/90 text-brand-stone px-6 py-2.5 rounded-full font-medium transition-all shadow-md hover:shadow-lg flex items-center gap-2"
              >
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden bg-brand-stone border-b border-brand-charcoal/10 absolute w-full left-0 shadow-lg">
            <div className="px-4 pt-2 pb-6 space-y-2 flex flex-col">
              <button onClick={() => scrollToSection('home')} className="block w-full text-left px-3 py-3 text-base font-medium text-brand-charcoal/80 hover:text-[var(--color-brand-sage)] hover:bg-brand-stone/50 rounded-md">Home</button>
              <button onClick={() => scrollToSection('model')} className="block w-full text-left px-3 py-3 text-base font-medium text-brand-charcoal/80 hover:text-[var(--color-brand-sage)] hover:bg-brand-stone/50 rounded-md">Our Model</button>
              <button onClick={() => scrollToSection('about')} className="block w-full text-left px-3 py-3 text-base font-medium text-brand-charcoal/80 hover:text-[var(--color-brand-sage)] hover:bg-brand-stone/50 rounded-md">About Us</button>
              <button onClick={() => scrollToSection('contact')} className="block w-full text-left px-3 py-3 text-base font-medium text-brand-charcoal/80 hover:text-[var(--color-brand-sage)] hover:bg-brand-stone/50 rounded-md">Contact</button>
              <button
                onClick={() => scrollToSection('apply')}
      ...
      {/* EDUCATIONAL MODEL */}
      <section id="model" className="py-24 bg-brand-stone">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {/* Card 1 */}
            <div className="bg-[var(--color-brand-stone)] rounded-2xl p-8 border border-brand-charcoal/10 shadow-sm hover:shadow-md transition-shadow group relative overflow-hidden">
              <div className="absolute top-0 right-0 w-32 h-32 bg-brand-stone rounded-bl-full -mr-16 -mt-16 transition-transform group-hover:scale-110"></div>
              <div className="bg-brand-stone w-14 h-14 rounded-xl shadow-sm flex items-center justify-center mb-6 relative z-10 text-[var(--color-brand-sage)]">
                <BookOpen className="h-7 w-7" />
              </div>
              <h4 className="text-xl font-bold text-brand-charcoal mb-3 relative z-10">Classical Foundation & Literacy</h4>
              <p className="text-brand-charcoal/70 leading-relaxed relative z-10">
                A strong emphasis on literacy, logic, and classical educational roots. We build robust foundational skills that empower critical thinking.
              </p>
            </div>

            {/* Card 2 */}
            <div className="bg-[var(--color-brand-sage)] rounded-2xl p-8 border border-brand-sage/30 shadow-sm hover:shadow-md transition-shadow group relative overflow-hidden">
              <div className="absolute top-0 right-0 w-32 h-32 bg-brand-stone rounded-bl-full -mr-16 -mt-16 transition-transform group-hover:scale-110"></div>
              <div className="bg-brand-stone w-14 h-14 rounded-xl shadow-sm flex items-center justify-center mb-6 relative z-10 text-brand-sage">
                <Lightbulb className="h-7 w-7" />
              </div>
              <h4 className="text-xl font-bold text-brand-charcoal mb-3 relative z-10">Hybrid & Project-Based</h4>
              <p className="text-brand-charcoal/70 leading-relaxed relative z-10">
                Core mastery-based academics combined with dynamic, project-based enrichment. Students apply what they learn to real-world scenarios.
              </p>
            </div>
            ...
            <div className="bg-[var(--color-brand-stone)] rounded-2xl p-8 border border-brand-charcoal/10 shadow-sm hover:shadow-md transition-shadow group relative overflow-hidden">
              ...
              <h4 className="text-xl font-bold text-brand-charcoal mb-3 relative z-10">Individualized Learning</h4>
              <p className="text-brand-charcoal/70 leading-relaxed relative z-10">
                Small teacher-to-student ratios focused on individualized pacing and deep character formation tailored to each unique child.
              </p>
            </div>
          </div>
        </div>
      ...
      {/* ABOUT US */}
      <section id="about" className="py-24 bg-[var(--color-brand-stone)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row items-center gap-16">
            <div className="lg:w-1/2 w-full">
              <div className="relative rounded-2xl overflow-hidden aspect-[4/3] bg-brand-stone/90 shadow-2xl border border-brand-charcoal/10">
                <Image src="/about_us_team.png" alt="Cephas Institute Educator Team" fill className="object-cover hover:scale-105 transition-transform duration-700" />
              </div>
            </div>

            <div className="lg:w-1/2 w-full">
              <h2 className="text-sm font-bold tracking-widest text-[var(--color-brand-sage)] uppercase mb-3">About Us</h2>
              <h3 className="text-3xl md:text-4xl font-bold text-brand-charcoal mb-6">Our Vision & Team</h3>
              <div className="space-y-6 text-lg text-brand-charcoal/70 leading-relaxed">
                <p>
                  At Cephas Institute, we believe that education is not merely the impartation of facts, but the formation of the whole person. Our mission is to partner with families to cultivate wisdom, virtue, and purpose in every student.
                </p>
                <p>
                  Founded by educators and community leaders who saw the need for a more intentional approach to learning, we are building a team of passionate, highly qualified mentors dedicated to guiding your child's academic and spiritual journey.
                </p>
                <ul className="space-y-3 mt-4">
                  {[
                    ...
                    "Focus on holistic character development"
                  ].map((item, i) => (
      ...
      <footer className="bg-brand-charcoal text-gray-400 py-12 border-t border-gray-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex flex-col md:flex-row justify-between items-center gap-6">
          ...
          <div className="flex gap-6">
            <button onClick={() => scrollToSection('home')} className="hover:text-brand-stone transition-colors text-sm">Home</button>
            <button onClick={() => scrollToSection('model')} className="hover:text-brand-stone transition-colors text-sm">Our Model</button>
            <button onClick={() => scrollToSection('about')} className="hover:text-brand-stone transition-colors text-sm">About Us</button>
            <button onClick={() => scrollToSection('contact')} className="hover:text-brand-stone transition-colors text-sm">Contact</button>
          </div>
          <p className="text-sm">
